...
public class Deletion_of_SMI_Endorsement_Reg extends BaseClass {

	@Test(dataProvider = "Deletion_of_SMI_Reg")
...
//		Login_Page objec = new Login_Page();
...
//		Delete SMI
		webDriverWait(ExpectedConditions.elementToBeClickable(EIP.delete_SMI()));
		EIP.delete_SMI().click();

		Thread.sleep(3000);

//		Get Deleted Premium
		webDriverWait(ExpectedConditions.visibilityOf(EIP.deleted_SMI_Premium()));
		String deleted_SMI_Premium = EIP.deleted_SMI_Premium().getText();
		System.out.println("Deleted SMI Premium is: " + deleted_SMI_Premium);

//		Click Proceed Button
		webDriverWait(ExpectedConditions.elementToBeClickable(EIP.proceed_Button()));
		EIP.proceed_Button().click();
		try {
			webDriverWait(ExpectedConditions.elementToBeClickable(EIP.ok_Button()));
			EIP.ok_Button().click();
		} catch (Exception e) { }
		Thread.sleep(10000);
//		get Policy Fess 
...
//		EIP.special_Tty_No_Option().click();
		...
		if (gross_premium.contains(deleted_SMI_Premium)) {
			Assert.assertEquals(gross_premium, deleted_SMI_Premium);
		...
		if (Mode_Of_Pay.equals("Credit") || Mode_Of_Pay.equals("Debit")) {
			try {
