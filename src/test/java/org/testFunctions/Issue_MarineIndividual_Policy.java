...
public class Issue_MarineIndividual_Policy extends BaseClass {
	...
	@Test(dataProvider = "Marine_Individual")
	public void tc_001(String S_No, String Policy_Type, String Types_of_Policy, String Insured_Query, String Insured_ID,
			String Quotation_Validity_Days, String Business_Source, String Co_Insurance_Share_Percentage,
			String Sum_Insured_Currency, String Premium_Currency, String Contact_Number, String Business_Occupation,
			String Territorial_Limits, String Introducer_Name, String Processor_Name, String Incoterms,
			String Variance_Percentage, String MI_Bank_YN, String Inward_SI, String Inward_Premium,
			String SumInsured_Value, String SI_Rate, String SMI_Discount, String SMI_Loadings, String Discount_Rate,
			String Loading_Rate, String Policy_Discount, String Policy_Discount_Rate, String Policy_Loading,
			String Policy_Loading_Rate, String Policy_Deductible, String Policy_Deductible_Calctype,
			String Policy_Deductible_RateValue, String Coinsurer_Name_Query, String Coinsurer_ID,
			String Coinsurer_Share, String Split_YN, String Add_Insured_Query, String Add_Insured_ID, String Ins_Type,
			String AddIns_MobileNo, String AddIns_EmailID, String AddIns_IDType, String AddIns_IdNo,
			...
			String Remarks, String FAC_Participant, String RI_Status, String Approve_Policy, String Run_Flag)
...
//under writing page
		...
		// Click Marine Individual
		webDriverWait(ExpectedConditions.elementToBeClickable(uwp.Marine_Individual()));
		uwp.Marine_Individual().click();
...
//		Get ID Card Number
		...
		String frm_date = getAtrributeValue(cust.policy_From_Date(), "value");
		System.out.println("Individual Policy Start Date: " + frm_date);

//		Enter Quotation Validity Days
//		webDriverWait(ExpectedConditions.visibilityOf(MP.quotation_Valid_Days()));
//		MP.quotation_Valid_Days().clear();
//		MP.quotation_Valid_Days().sendKeys(Quotation_Validity_Days);
...
//Risk info page	
		...
		LocalDateTime dateTime = LocalDateTime.parse(frm_date, inputFormatter);
		LocalDateTime postDateTime = dateTime.plus(1, ChronoUnit.MONTHS);
		DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("MM/yyyy");
		String ShipmentDate = postDateTime.format(outputFormatter);
		MP.MOC_Declaration_Mon_Year().sendKeys(ShipmentDate, Keys.TAB);
		System.out.println("Marine Cerificate Shipment Date: " + ShipmentDate);

//		webDriverWait(ExpectedConditions.elementToBeClickable(MP.Conveyance_Type()));
//		selectByVisibleText(MP.Conveyance_Type(), Conveyance);
//		System.out.println("conveyance type: " + Conveyance);

		webDriverWait(ExpectedConditions.elementToBeClickable(MP.Conveyance_Type()));
		Select dropdown = new Select(MP.Conveyance_Type());
		List<WebElement> Conveyancetype = dropdown.getOptions();
		List<WebElement> validOptions = new ArrayList<>();
		for (WebElement option : Conveyancetype) {
		...
		if (!validOptions.isEmpty()) {
			...
			int randomIndexOfConveyanceType = random.nextInt(validOptions.size());
			dropdown.selectByIndex(Conveyancetype.indexOf(validOptions.get(randomIndexOfConveyanceType)));
			String ConveyanceType = validOptions.get(randomIndexOfConveyanceType).getText();
			System.out.println("Conveyance Type: " + ConveyanceType);
		}

//		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MOC_Cer_PortsFM()));
//		selectByVisibleText(MP.MOC_Cer_PortsFM(), Port_FM);
//		System.out.println("Import from: " + Port_FM);

		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MOC_Cer_PortsFM()));
		Select dropdown1 = new Select(MP.MOC_Cer_PortsFM());
		List<WebElement> PortsFM = dropdown1.getOptions();
		List<WebElement> Options = new ArrayList<>();
		for (WebElement option : PortsFM) {
			if (!option.getText().toLowerCase().contains("select")) {
				Options.add(option);
			}
		}
		if (!Options.isEmpty()) {
			Random random = new Random();
			int randomIndexOfPortsFM = random.nextInt(Options.size());
			dropdown1.selectByIndex(PortsFM.indexOf(Options.get(randomIndexOfPortsFM)));
			String Portsform = Options.get(randomIndexOfPortsFM).getText();
			System.out.println("Ports from: " + Portsform);
		}

//		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MOC_Cer_PortsTO()));
//		selectByVisibleText(MP.MOC_Cer_PortsTO(), Port_TO);
//		System.out.println("Export To: " + Port_TO);

		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MOC_Cer_PortsTO()));
		Select dropdown11 = new Select(MP.MOC_Cer_PortsTO());
		List<WebElement> PortsTo = dropdown11.getOptions();
		List<WebElement> Options1 = new ArrayList<>();
		for (WebElement option : PortsTo) {
			if (!option.getText().toLowerCase().contains("select")) {
				Options1.add(option);
			}
		}
		if (!Options1.isEmpty()) {
			Random random = new Random();
			int randomIndexOfPortsTo = random.nextInt(Options1.size());
			dropdown11.selectByIndex(PortsTo.indexOf(Options1.get(randomIndexOfPortsTo)));
			String Portsto = Options1.get(randomIndexOfPortsTo).getText();
			System.out.println("Ports To: " + Portsto);
		}

		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_Incoterms()));
		MP.MI_Incoterms().sendKeys(Incoterms);
		System.out.println("Incoterms: " + Incoterms);

//		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_Incoterms()));
//		Select dropdown12 = new Select(MP.MI_Incoterms());
//		List<WebElement> Incoterms = dropdown12.getOptions();
//		List<WebElement> Options11 = new ArrayList<>();
//		for (WebElement option : PortsTo) {
//			if (!option.getText().toLowerCase().contains("select")) {
...
//			Random random = new Random();
//			int randomIndexOfIncoterms= random.nextInt(Options11.size());
//			WebElement selectedOption = Options11.get(randomIndexOfIncoterms);
//			dropdown.selectByVisibleText(selectedOption.getText());
//			String incoterms = selectedOption.getText();
//			System.out.println("incoterms: " + incoterms);
//		}

		rb.delay(5000);
		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_Variance()));
		MP.MI_Variance().sendKeys(Variance_Percentage);
		...
		MP.Marine_LCNO().sendKeys("876587");
...
//click Add Goods
		...
		javaScribtClick(MP.MOC_Cer_Goods());

		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_1SMI_Checkbox()));
		javaScribtClick(MP.MI_1SMI_Checkbox());

		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_1SMI_Clauses()));
		selectByIndex(MP.MI_1SMI_Clauses(), 1);

		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_1SMI_SIvalue()));
		MP.MI_1SMI_SIvalue().sendKeys(SumInsured_Value);

		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_1SMI_Rate()));
		MP.MI_1SMI_Rate().sendKeys(SI_Rate);

		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_2SMI_Checkbox()));
		MP.MI_2SMI_Checkbox().click();

		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_2SMI_Clauses()));
		selectByIndex(MP.MI_2SMI_Clauses(), 2);

		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_2SMI_SIvalue()));
		MP.MI_2SMI_SIvalue().sendKeys(SumInsured_Value);

		webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_2SMI_Rate()));
		MP.MI_2SMI_Rate().sendKeys(SI_Rate);

		webDriverWait(ExpectedConditions.elementToBeClickable(ris.save_Button()));
		ris.save_Button().click();

		rb.delay(5000);
		webDriverWait(ExpectedConditions.elementToBeClickable(ris.Risk_Row()));
		javaScribtClick(ris.Risk_Row());

		rb.delay(5000);
		// Add Discounts and Loadings to the SMI
		javaScribtClick(ris.SMI_DL_Icon());
		...
		try {
			...
			webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_SMI_Discount_Checkbox()));
			MP.MI_SMI_Discount_Checkbox().click();

			webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_SMI_Discount_rate()));
			MP.MI_SMI_Discount_rate().sendKeys(Discount_Rate, Keys.TAB);
			...
			webDriverWait(ExpectedConditions.elementToBeClickable(ris.SMI_No_Dis_Loadings()));
			String text = ris.SMI_No_Dis_Loadings().getText();
			System.out.println("SMI Discounts: "+text);
		...
		try {
			...
			rb.delay(5000);
			webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_SMI_Loading_Checkbox()));
			MP.MI_SMI_Loading_Checkbox().click();

			webDriverWait(ExpectedConditions.elementToBeClickable(MP.MI_SMI_Loading_Rate()));
			MP.MI_SMI_Loading_Rate().sendKeys(Loading_Rate, Keys.TAB);
			...
			webDriverWait(ExpectedConditions.elementToBeClickable(ris.SMI_No_Dis_Loadings()));
			String text = ris.SMI_No_Dis_Loadings().getText();
			System.out.println("SMI Loadings: "+text);
...
// Risk Premium and Annual Premium
		...
		annual_Sum_Insured_Premium = MP.MC_Shipment_AnnualPremium().getText();
...
//				}
		...
		Select dropdown2 = new Select(apin.Surveyor_Name());
		List<WebElement> SurveyorName = dropdown2.getOptions();
		...
		if (!validOptions1.isEmpty()) {
			...
			dropdown2.selectByIndex(SurveyorName.indexOf(validOptions1.get(randomIndexOfSurveyorName)));
		...
		// scrollDownJavaSc(MP.Policy_Document_Dropdown());
...
//		Click Proceed
		rb.delay(3000);
		webDriverWait(ExpectedConditions.visibilityOf(cust.proceed_Button()));
...
// excess of treaty validation
					webDriverWait(ExpectedConditions.visibilityOf(ra.Excess_Treaty_Percent()));
					String excess_Percentage = ra.Excess_Treaty_Percent().getText();
					System.out.println("Excess Treaty Percentage: "+excess_Percentage);
					
					try {
					    BigDecimal decimalValue = new BigDecimal(excess_Percentage);
					    //  If value is NOT zero, perform validation
					    if (decimalValue.compareTo(BigDecimal.ZERO) != 0) {
					        System.out.println("❗ Value is non-zero, needs validation: " + decimalValue);
					        
					    webDriverWait(ExpectedConditions.elementToBeClickable(ra.prop_FAC_Button()));
					    ra.prop_FAC_Button().click();
					    
					    webDriverWait(ExpectedConditions.elementToBeClickable(ra.FAC_Percentage_Field()));
					    ra.FAC_Percentage_Field().sendKeys(excess_Percentage);
						...
						webDriverWait(ExpectedConditions.elementToBeClickable(ri.Select_FAC_Participant_Name()));
						ri.Select_FAC_Participant_Name().click();
						String Fac_Participant = ri.FAC_Participant_Name().getAttribute("value");
						System.out.println("FAC Participant Name = " + Fac_Participant);
						
						webDriverWait(ExpectedConditions.elementToBeClickable(ri.FAC_Participant_Share_Percentage()));
						ri.FAC_Participant_Share_Percentage().sendKeys(excess_Percentage, Keys.TAB);

						webDriverWait(ExpectedConditions.elementToBeClickable(ri.FAC_Participant_Save_Close()));
						ri.FAC_Participant_Save_Close().click();
						
						webDriverWait(ExpectedConditions.elementToBeClickable(ri.RI_Allocation()));
						ri.RI_Allocation().click();					        
					    } 
