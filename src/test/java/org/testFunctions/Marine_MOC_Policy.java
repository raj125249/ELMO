...
public class Marine_MOC_Policy extends BaseClass {
...
//				Click Yes button
					...
					webDriverWait(ExpectedConditions.elementToBeClickable(ra.RI_Confirmed_Msg()));
					String RI_Msg = ra.RI_Confirmed_Msg().getText();
					System.out.println("RI confirmed : " + RI_Msg);
					...
					webDriverWait(ExpectedConditions.elementToBeClickable(ra.logout_Button()));
					javaScribtClick(ra.logout_Button());
...
//				Enter Quote Number
		...
		if (MOC_Policy_Number == null) {
			...
			webDriverWait(ExpectedConditions.elementToBeClickable(uwp.Open_Policy_Number()));
			uwp.Open_Policy_Number().sendKeys(MOC_Policy_Number);
			javaScribtClick(uwp.Open_Policy_number_click());
		}
		rb.delay(2000);
...
//Enter Proceed Button
		...
		LocalDate nextDate2 = nextDate1.toLocalDate();
		LocalDate nextDay = nextDate2.plus(1, ChronoUnit.DAYS);
...
//		System.out.println("Add Insured Details saved successfully");
		...
		wait1.until(ExpectedConditions.visibilityOf(apin.Surveyor_Name()));
		// Create a Select object for the dropdown
...
//Certificate Generation starts
		...
		if (Special_Tty.equals("Y")) {
			...
			// Click Special Treaty Yes Option
			webDriverWait(ExpectedConditions.elementToBeClickable(ri.special_Tty_Yes_Option()));
			javaScribtClick(ri.special_Tty_Yes_Option());
...
//include FAC in policy
		...
		javaScribtClick(ri.proceed_Button());
...
// Approve as Policy in Search page.
		if (Approve_Policy.equals("Yes")) {
			...
			try {
				...
				ra.Summary_Policy_PrintDocs().click();
				rb.delay(3000);
				List<WebElement> listCheckBoxes11 = ra.Summary_Quotation_Checkbox();
				...
				if (ra.Quotation_SI_WFMSG().isDisplayed() || ra.Quotation_Information_RA().isDisplayed()) {
					...
					rb.delay(3000);
					List<WebElement> listCheckBoxes121 = GSP.Print_CheckBox();
					System.out.println("Print document List: " + listCheckBoxes121.size());
					for (int i = 0; i < listCheckBoxes121.size(); i++) {
						listCheckBoxes121.get(i).click();
					}
